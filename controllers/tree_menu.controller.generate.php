<?php
    ...
    class tree_menuControllerGenerate
    {
        protected $tree_menu;
        var $modulename = "tree_menu";
        var $dbh;
        var $limit = 20;
        var $user = "None";
        var $ip = "";
        ...
        var $lastID = "";
        var $toolsController;
        function __construct($tree_menu, $dbh) {
            $this->modulename = strtoupper($this->modulename);
            $this->tree_menu = $tree_menu;
            $this->dbh = $dbh;            
	    ...
	    $sql .= "`parent_id` ";
            ...
            $sql .= " VALUES (";
	    $sql .= " null,";
	    $sql .= "'".$this->toolsController->replacecharSave($this->tree_menu->getNama_menu(), $this->dbh)."',";
	    $sql .= "'".$this->toolsController->replacecharSave($this->tree_menu->getParent_id(), $this->dbh)."' ";
        ...
        function updateData(){
            $datetime = date("Y-m-d H:i:s");
            $sql = "UPDATE tree_menu SET ";
	    $sql .= "`id` = '".$this->toolsController->replacecharSave($this->tree_menu->getId(),$this->dbh)."',";
	    $sql .= "`nama_menu` = '".$this->toolsController->replacecharSave($this->tree_menu->getNama_menu(),$this->dbh)."',";
	    $sql .= "`parent_id` = '".$this->toolsController->replacecharSave($this->tree_menu->getParent_id(),$this->dbh)."' ";
            $sql .= "WHERE id = '".$this->tree_menu->getId()."'";                
            $execute = $this->dbh->query($sql);
        }
        
        function deleteData($id){
            $sql = "DELETE FROM tree_menu WHERE id = '".$id."'";
            $execute = $this->dbh->query($sql);
        }
        
        function showData($id){
            $sql = "SELECT * FROM tree_menu WHERE id = '".$this->toolsController->replacecharFind($id,$this->dbh)."'";

            $row = $this->dbh->query($sql)->fetch();
            $this->loadData($this->tree_menu, $row);
            
            return $this->tree_menu;
        }
        
        function checkData($id){
            $sql = "SELECT count(*) FROM tree_menu where id = '".$id."'";
            $row = $this->dbh->query($sql)->fetch();
            return $row[0];
        }
        ...
        function showDataWhereQuery(){
            ...
            if ($bsearch) {
               ...
               $where .= "       or  nama_menu like '%".$search."%'";
               $where .= "       or  parent_id like '%".$search."%'";
        ...
        function findDataWhere($where){
            $sql = "SELECT * FROM tree_menu  ".$where;
            $sql .= " ORDER BY id";
            return $sql;
        }
        function findCountDataWhere($where){
            $sql = "SELECT count(id)  FROM tree_menu  ".$where;
            $sql .= " ORDER BY id";
            return $sql;
        }
        ...
        function createList($sql){
            $tree_menu_List = array();
            foreach ($this->dbh->query($sql) as $row) {
                    $tree_menu_ = new tree_menu();
                    $this->loadData($tree_menu_, $row);
                    $tree_menu_List[] = $tree_menu_;
            }
            return $tree_menu_List;            
        }

                
        function loadData($tree_menu,$row){
	    $tree_menu->setId($row['id']);
	    $tree_menu->setNama_menu($row['nama_menu']);
	    $tree_menu->setParent_id($row['parent_id']);
        ...
        function showAll(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                ...
                $tree_menu_list = $this->showDataAllLimit();
                ...
                require_once './views/tree_menu/tree_menu_list.php';
            }else{
        ...
        function showAllJQuery(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                ...
                $isexport = $this->isexport ;
                $isimport = $this->isimport;
                require_once './views/tree_menu/tree_menu_jquery_list.php';
            }else{
                echo "You cannot access this module";
            }
        }
        ...
        function showDetail(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                $id = $_GET['id'];
                $tree_menu_ = $this->showData($id);
                require_once './views/tree_menu/tree_menu_detail.php';
            }else{
                echo "You cannot access this module";
            }
        }
        function showDetailJQuery(){
            if ($this->ispublic || $this->isadmin || $this->isread ){
                $id = $_GET['id'];
                $tree_menu_ = $this->showData($id);
                require_once './views/tree_menu/tree_menu_jquery_detail.php';
            }else{
                echo  "You cannot access this module";
            }
        }
        ...
        function showForm(){
            if ($this->ispublic || $this->isadmin || ($this->isread && $this->isupdate)){
                ...
                $tree_menu_ = $this->showData($id);
                require_once './views/tree_menu/tree_menu_form.php';
            }else{
                echo "You cannot access this module";
            }
        }
        ...
        function showFormJQuery(){
            if ($this->ispublic || $this->isadmin || ($this->isread && $this->isupdate)){
                ...
                $search = isset($_REQUEST["search"]) ? $_REQUEST["search"] : "";
                $tree_menu_ = $this->showData($id);
                require_once './views/tree_menu/tree_menu_jquery_form.php';
            }else{
                echo "You cannot access this module";
            }
        }        
	    ...
	    $this->tree_menu->setId($id);
	    $this->tree_menu->setNama_menu($nama_menu);
	    $this->tree_menu->setParent_id($parent_id);            
            $this->saveData();
        }
        public function saveData(){
            $check = $this->checkData($this->tree_menu->getId());
            if($check == 0){
        ...
        public function getTree_menu() {
            return $this->tree_menu;
        }
        public function setTree_menu($tree_menu) {
            $this->tree_menu = $tree_menu;
        }
